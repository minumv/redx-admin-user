...
export const google = async(req, res, next)=> {
    try{
        ...
        if (user){
            ...
            const generatedPassword = Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8) // 8 digit + 8 digit
            ...
            const { password: hashedPasseword2, ...rest} = newUser._doc;
            const expiryDate = new Date(Date.now() + 3600000);
